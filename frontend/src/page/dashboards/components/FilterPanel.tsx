...
export default function FilterPanel({ onFilterChange }: FilterPanelProps) {
  ...
  const handleApply = () => {
    ...
    const cleanFilters = Object.fromEntries(
      Object.entries(localFilter).filter(([ v]) => v !== undefined && v !== "")
    ) as Filters;
  ...
  return (
    <div className="bg-white rounded-2xl flex-wrap w-full p-5 shadow-sm">
      ...
      <form className="flex flex-wrap gap-5 items-end" onSubmit={(e) => {
        ...
        <Button  className="cursor-pointer bg-red-500 hover:bg-red-300 text-white" variant="destructive" type="button" onClick={handleReset}>
          Reset
        </Button>
      </form>
    </div>
