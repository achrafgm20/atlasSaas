...
type FilterOption = string | FilterOptionObject;

type FilterCategory = 'Brand' | 'Model' | 'Grade' | 'Storage' | 'Color';
...
const Filter = () => {
  ...
  const selectOption = (category: FilterCategory, value: string) => {
  ...
  const getOptionIcon = (option: FilterOption): string | null | undefined => {
    return typeof option === 'string' ? undefined : option.icon;
  };

  const getOptionColor = (option: FilterOption): string | null | undefined => {
      ...
      {showFilters && (
        <div className="flex flex-wrap items-center gap-4 animate-in fade-in slide-in-from-top-2">
          ...
          <div className="flex flex-wrap gap-3 relative">
            {(Object.keys(filterOptions) as FilterCategory[]).map((key) => (
