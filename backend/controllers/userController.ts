...
export const editSellerProfile = asyncHandler(
  async (req: Request, res: Response) => {
    try {
      ...
      res
        .status(200)
        .json({ message: "seller edited succefully", editedProfile });
    } catch (err) {
      res
        .status(400)
        .json({ message: "error while editing seller s profile ", err });
      console.error(err);
    }
  },
);

export const checkAllSellersStatus = asyncHandler(
  async (req: Request, res: Response) => {
    try {
...
export const refreshStripeOnboarding = asyncHandler(
  async (req: Request, res: Response) => {
    const userId = req.query.userId as string;
...
export const getAllUsers = asyncHandler(async (req: Request, res: Response) => {
  const users = await User.find({ role: { $in: ["Buyer", "Seller"] } }).select(
    "-password",
  );
...
export const editStatusSeller = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    await seller.save();
    try {
      await sendSellerStatusEmail(seller.email, statutCompte);
    } catch (err) {
...
export const filterBuyerSellers = asyncHandler(
  async (req: Request, res: Response) => {
...
export const manualSyncStripe = asyncHandler(
  async (req: Request, res: Response) => {
